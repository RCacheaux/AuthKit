...
- (void)authenticate {
  ...
  NSURL *tokenURL = [NSURL URLWithString:@"https://www.linkedin.com/uas/oauth2/accessToken"];
  GTMOAuth2Authentication *auth =
      [GTMOAuth2Authentication authenticationWithServiceProvider:@"LinkedIn"
                                                        tokenURL:tokenURL
                                                     redirectURI:@"http://www.linkedin.com"
                                                        clientID:@""
                                                    clientSecret:@""];
  auth.scope = @"";
  auth.additionalGrantTypeRequestParameters =
      @{@"authorization_code" : @{ @"state" : @"98797542" }};
  
  
  NSURL *authURL = [NSURL URLWithString:@"https://www.linkedin.com/uas/oauth2/authorization"];
